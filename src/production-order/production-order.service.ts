...
export class ProductionOrderService {
  ...
  private async buildMaterialSnapshot(
    ...
    if (specs.outerLeather?.sqft) {
      const material = await this.findMaterialByType('OUTER_LEATHER', specs.outerLeather);
      const price = material ? Number(material.unitCost) : 0;
      snapshots.push({
        material_id: material?.id || null,
        material_name: specs.outerLeather.material_name || 'Outer Leather',
        qty_per_unit: Number(specs.outerLeather.sqft) * (1 + (specs.outerLeather.wastage_rate || 0)),
        qty_consumed: Number(specs.outerLeather.sqft) * quantity * (1 + (specs.outerLeather.wastage_rate || 0)),
        unit: 'sqft',
    ...
    if (specs.liningLeather?.sqft) {
      const material = await this.findMaterialByType('LINING_LEATHER', specs.liningLeather);
      const price = material ? Number(material.unitCost) : 0;
      snapshots.push({
        material_id: material?.id || null,
    ...
    if (specs.interlining?.sqft) {
      const material = await this.findMaterialByType('INTERLINING', specs.interlining);
      const price = material ? Number(material.unitCost) : 0;
      snapshots.push({
        material_id: material?.id || null,
