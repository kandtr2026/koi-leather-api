...
export class RawMaterialController {
  ...
  @Get()
  @ApiOperation({ summary: 'List all raw materials (filterable by type/supplier)' })
  @ApiQuery({ name: 'type', required: false, enum: ['OUTER_LEATHER', 'LINING_LEATHER', 'INTERLINING', 'THREAD', 'BUCKLE', 'HARDWARE'] })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get raw material details with recent transactions' })
  findOne(@Param('id', ParseUUIDPipe) id: string) {
    return this.materialService.findById(id);
  }

  @Patch(':id')
  @ApiOperation({ summary: 'Update raw material (can change price — affects NEW orders only)' })
